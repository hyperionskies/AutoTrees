...
def read(line,mydict,years):
	...
	if not year in mydict:
		mydict[year] = 0
	mydict[year] = mydict[year] + trees

mydict = {}
years = []

db_file = open(os.path.dirname(__file__)+'\\matrix_file_nr.txt','r')
test = db_file.readline().rstrip()
while not test is '':
	read(test,mydict,years)
	test = db_file.readline().rstrip()
...
path = basedir + '\\matrix_file_nr.txt'


#--- Output folder


renderpath = basedir + '\\blender.avi'


#--- Color key

colorkey = basedir + '\\colorkey.txt'


#--- year change

yearjump = find_gcd(years)
...
treelist = []
for y in mydict:
	treelist.append(mydict[y])

maxtrees = max(treelist)

while((maxcoord*maxcoord*4) < maxtrees):
	maxcoord = maxcoord + 50
	cscale = cscale + 1
...
hold = {'year':{0:{}},'loc':{},'cohorts':[],'species':[],'cmap':[],'colors':colors,'oof':[], 'maxyear':max(mydict)}
